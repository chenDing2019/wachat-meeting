...
import tech.chending.wechat.pojo.resp.Article;
import tech.chending.wechat.pojo.resp.MusicMessageResp;
import tech.chending.wechat.pojo.resp.NewsMessageResp;
import tech.chending.wechat.pojo.resp.TextMessageResp;
...
public class MessageUtil {
    ...
    public static String musicMessageToXml(MusicMessageResp musicMessage) {
        XSTREAM.alias("xml", MusicMessageResp.class);
        ...
        public HierarchicalStreamWriter createWriter(Writer out) {
            return new PrettyPrintWriter(out) {
                ...
                public void startNode(String name, Class clazz) {
                	//过滤两个特殊的标签，这两个标签首字母不需要大写
					if ("xml".equals(name) || "item".equals(name)) {
						super.startNode(name, clazz);
					} else {
						String substring = name.substring(0, 1);
						name = name.replace(substring, substring.toUpperCase());
						super.startNode(name, clazz);
					}
                }
     ...
     * @return
     */
    public static String getTextMsgXml(String fromUserName,String toUserName,String respContent) {
